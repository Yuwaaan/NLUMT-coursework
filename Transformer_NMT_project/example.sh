...
python translate.py \
    ...
    --output "${TEST_EN_PRED}"
# python translate.py --checkpoint-path question5_lexical/checkpoint_best.pt --output question5_lexical_translate.txt
# python translate.py --checkpoint-path question7/checkpoint_best.pt --output question7_translate.txt

# python train.py --save-dir question7 --arch transformer

## Calculate BLEU score for model outputs
perl multi-bleu.perl -lc ${TEST_EN_GOLD} < ${TEST_EN_PRED} | tee "${EXP_ROOT}/bleu.txt"
# perl multi-bleu.perl -lc europarl_raw/test.en < question5_lexical_translate.txt
# perl multi-bleu.perl -lc europarl_raw/test.en < question7_translate.txt